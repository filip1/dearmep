...
import {
  CallingErrorType,
  CallingService,
} from 'src/app/services/calling/calling.service';
...
export class SetupStepComponent implements OnInit {
  ...
  private initiateCall(destinationID: string, language: string) {
    this.callingService
      .setUpCall(destinationID, language)
      .pipe(take(1))
      .subscribe({
        next: () => {
          this.routingStateManager.goToFeedback();
        },
        error: (err: CallingErrorType) => {
          let errorDialogResult: Observable<void> | undefined;

          if (
            err === CallState.CallingUserFailed ||
            err === CallState.CallAborted
          ) {
            errorDialogResult = this.errorService.displayErrorDialog(
