...
export class ErrorService {
  ...
  public displayErrorDialog(
    ...
    buttonAcceptText = 'error.errorDialogOK',
  ...
  private showSnackBar(translationKey: string) {
    ...
    const errorText =
      translocoService?.translate(translationKey) || translationKey;
  ...
  private getTranslocoService(): TranslocoService | undefined {
    if (!this._translocoServiceLazy) {
      this._translocoServiceLazy =
        this.injector.get(TranslocoService, undefined, { optional: true }) ||
        undefined;
    }
