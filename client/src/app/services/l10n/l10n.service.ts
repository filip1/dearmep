...
export class L10nService {
  ...
  constructor(
    ...
    this.userSelectedCountry$ = new BehaviorSubject<string | undefined>(
      this.configService.getSelectedCountry()
    );

    this.detectedCountry$ = this.configService
      .getConfig$()
      .pipe(map(c => c.location.country));
    this.detectedLanguage$ = this.configService
      .getConfig$()
      .pipe(map(c => c.language.recommended));

    this.availableCountries$ = this.configService
      .getConfig$()
      .pipe(map(c => c.location.available));
    this.availableLanguages$ = this.configService
      .getConfig$()
      .pipe(map(c => c.language.available));
    ...
    this.country$ = combineLatest([
      ...
      map(
        ([
          ...
          return (
            this.mustBePartOf(userSelectedCountry, availableCountries) ||
            apiDetectedCountry ||
  ...
  public getLanguage$(): Observable<string | undefined> {
    return combineLatest([
      ...
      map(([userSelectedLang, apiDetectedLang, availableLanguages]) => {
        return (
          this.mustBePartOf(userSelectedLang, availableLanguages) ||
          apiDetectedLang
        );
      })
    );
