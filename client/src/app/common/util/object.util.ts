export class ObjectUtil {
  ...
  private static applyValueRec(obj: Record<string, unknown>, keySectins: string[], value: unknown) {
    if (keySectins.length === 0) { return }
    const key = keySectins[0]

    if (keySectins.length === 1) { // last key-section reached => set value
      ...
      return
    } else { // not the last key-section => add empty object or array
      if (!(key in obj)) {
    ...
    this.applyValueRec(obj[key] as Record<string, unknown>, remainingKeySections, value)
  }
